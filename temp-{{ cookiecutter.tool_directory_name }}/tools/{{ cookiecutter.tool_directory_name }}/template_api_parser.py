...
from .importer import {{ cookiecutter.tool_class_name }}Importer

SEVERITIES = {1: "Info", 2: "Low", 3: "Medium", 4: "High", 5: "Critical"}
...
class {{ cookiecutter.tool_class_name }}Parser(object):
    ...
    def get_description_for_scan_types(self, scan_type):
        return "{{ cookiecutter.tool_name }} findings can be imported by API"

    def requires_file(self, scan_type):
        return False

    def requires_tool_type(self, scan_type):
        return "{{ cookiecutter.tool_name }}"

    def api_scan_configuration_hint(self):
        return 'the field <b>Service key 1</b> has to be set with the {{ cookiecutter.tool_name }} id.'

    def get_findings(self, file, test):
        if file:
            data = json.load(file)
        else:
            data = {{ cookiecutter.tool_class_name }}Importer().get_findings(test)
    ...
    def make_finding(self, test, vulnerability):
        ...
        finding.severity = SEVERITIES[vulnerability["severity"]]
