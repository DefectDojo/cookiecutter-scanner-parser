...
class Test{{ cookiecutter.tool_class_name }}Parser(TestCase):
    ...
    def test_{{ cookiecutter.tool_directory_name }}_parser_with_no_vuln_has_no_findings(self):
        testfile = open("unittests/scans/{{ cookiecutter.tool_directory_name }}/{{ cookiecutter.tool_directory_name }}_zero_vul.{{ cookiecutter.tool_file_type|lower }}")
    ...
    def test_{{ cookiecutter.tool_directory_name }}_parser_with_one_criticle_vuln_has_one_findings(self):
        testfile = open("unittests/scans/{{ cookiecutter.tool_directory_name }}/{{ cookiecutter.tool_directory_name }}_one_vul.{{ cookiecutter.tool_file_type|lower }}")
        parser = {{ cookiecutter.tool_class_name }}Parser()
        findings = parser.get_findings(testfile, Test())
        testfile.close()
    ...
    def test_{{ cookiecutter.tool_directory_name }}_parser_with_many_vuln_has_many_findings(self):
        testfile = open("unittests/scans/{{ cookiecutter.tool_directory_name }}/{{ cookiecutter.tool_directory_name }}_many_vul.{{ cookiecutter.tool_file_type|lower }}")
        parser = {{ cookiecutter.tool_class_name }}Parser()
        findings = parser.get_findings(testfile, Test())
        testfile.close()
        for finding in findings:
    ...
    def test_{{ cookiecutter.tool_directory_name }}_parser_empty_with_error(self):
        with self.assertRaises(ValueError) as context:
            testfile = open("unittests/scans/{{ cookiecutter.tool_directory_name }}/empty_with_error.{{ cookiecutter.tool_file_type|lower }}")
            parser = {{ cookiecutter.tool_class_name }}Parser()
            findings = parser.get_findings(testfile, Test())
            testfile.close()
            for finding in findings:
