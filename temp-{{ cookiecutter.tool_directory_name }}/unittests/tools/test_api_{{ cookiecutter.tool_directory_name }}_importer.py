...
from dojo.tools.{{ cookiecutter.tool_directory_name }}.importer import {{ cookiecutter.tool_class_name }}Importer


class Test{{ cookiecutter.tool_class_name }}Importer(TestCase):
    ...
    def test_prepare_client_do_not_match(self):
        ...
        with self.assertRaisesRegex(Exception, 'API Scan Configuration for {{ cookiecutter.tool_name }} and Product do not match.'):
            importer = {{ cookiecutter.tool_class_name }}Importer()
            importer.prepare_client(test_3)
    ...
    def test_prepare_client_more_than_one_configuration(self, mock_foo):
        ...
        with self.assertRaisesRegex(Exception, 'More than one Product API Scan Configuration has been configured, but none of them has been chosen.'):
            importer = {{ cookiecutter.tool_class_name }}Importer()
            importer.prepare_client(self.test)
    ...
    def test_prepare_client_one_product_configuration(self, mock_foo):
        ...
        importer = {{ cookiecutter.tool_class_name }}Importer()
        api, api_scan_configuration = importer.prepare_client(self.test)
    ...
    def test_prepare_client_one_test_configuration(self):
        importer = {{ cookiecutter.tool_class_name }}Importer()
        api, api_scan_configuration = importer.prepare_client(self.test_2)

        self.assertEqual(api_scan_configuration, self.api_scan_configuration_2)
        self.assertEqual(api.api_key, 'API_KEY')

    @patch('dojo.tools.{{ cookiecutter.tool_directory_name }}.importer.{{ cookiecutter.tool_class_name }}Importer.get_findings')
    def test_get_findings(self, mock_foo):
        mock_foo.return_value = self.findings

        importer = {{ cookiecutter.tool_class_name }}Importer()
        my_findings = importer.get_findings(self.test_2)
